...
# Change these
server '45.79.95.131', roles: [:web, :app, :db], primary: true

set :repo_url,        'git@github.com:CirQlar/Fixus.git'
set :application,     'fixus'
set :user,            'deploy'
...
namespace :deploy do
  ...
  task :reset => [:set_rails_env] do
    on primary fetch(:migration_role) do
      within release_path do
        with rails_env: fetch(:rails_env) do
          execute "export DISABLE_DATABASE_ENVIRONMENT_CHECK=1"
